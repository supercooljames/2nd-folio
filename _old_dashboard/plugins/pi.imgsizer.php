...
class imgsizer
{
	...
	function size()
	{
		...
		// -------------------------------------
		$src = ( ! $TMPL->fetch_param('image')) ? ( !$TMPL->fetch_param('src') ? '' : $TMPL->fetch_param('src') ) : $TMPL->fetch_param('image');
			$src = str_replace(SLASH, "/", $src); // clean up passed src param 
			
			/*debug*/
			$TMPL->log_item("imgsizer.user.src: ".$src);
		
			
		$base_path = ( ! $TMPL->fetch_param('base_path')) ? $base_path : $TMPL->fetch_param('base_path');
			$base_path = str_replace("\\", "/", $base_path);
			$base_path = $FNS->remove_double_slashes($base_path);
			$img['base_path']= $base_path;
			
			
		$remote = ( ! $TMPL->fetch_param('remote')) ? '' : $TMPL->fetch_param('remote');
		
		$cache = ( ! $TMPL->fetch_param('cache')) ? '' : $TMPL->fetch_param('cache');
		
		$refresh = ( ! $TMPL->fetch_param('refresh')) ? '1440' : $TMPL->fetch_param('refresh');
		...
		// -------------------------------------
		if(!$src){
			 $TMPL->log_item("imgsizer.Error: no src provided for imagesizer");
		...
		if (stristr($src, '<img'))
		{
			$TMPL->log_item("imgsizer.Error: you are trying to pass a img tag into imagesizer: no good");
			return $TMPL->no_results();			
		}
		...
		$img['src'] = $FNS->remove_double_slashes("/".$src);
		$img['base_path'] = $FNS->remove_double_slashes($base_path);
		
		/*debug*/
		$TMPL->log_item("imgsizer.img[src]: ".$img['src']);
		$TMPL->log_item("imgsizer.img[base_path]: ".$img['base_path']);
		...
		$img_full_path = $FNS->remove_double_slashes($img['base_path'].$img['src']);
		
		/*debug*/
		$TMPL->log_item("imgsizer.img_full_path: ".$img_full_path);
		
			
		// can we actually read this file?
		if(!is_readable($img_full_path)){	
		    $TMPL->log_item("imgsizer.Error: ".$img_full_path." image is not readable or does not exist");
			return $TMPL->no_results();
		}
		...
		$img['base_rel_path'] = $FNS->remove_double_slashes($src_pathinfo['dirname']."/");	
		
		/*debug*/
		$TMPL->log_item("imgsizer.img[base_rel_path]: ".$img['base_rel_path']);	

		// -------------------------------------
		// define all the file location pointers we will need 
		// -------------------------------------	
		$img_full_pathinfo = pathinfo($img_full_path);		
		$img['root_path'] = ( ! isset($img_full_pathinfo['dirname'])) ? '' : $FNS->remove_double_slashes($img_full_pathinfo['dirname']."/");
		...
		//$img['base_filename'] = $FNS->filename_security($img_full_pathinfo['filename']);
		$img['base_filename'] = str_replace(".".$img['extension'], "", $img_full_pathinfo['basename']);
		$img['base_filename'] = $FNS->filename_security($img['base_filename']);
		
		/*debug*/
		foreach($img_full_pathinfo as $key=>$value){
			$TMPL->log_item("imgsizer.full_pathinfo ".$key.": ".$value);
		}
		...
		if (!in_array($img['mime'], $okmime))
			{
			$TMPL->log_item("imgsizer.Error: image mime type does not match ok mime types");
			return $TMPL->no_results();
			}
		...
		// -------------------------------------
		$base_cache = $FNS->remove_double_slashes($base_path."/images/sized/");
		$base_cache = ( ! $TMPL->fetch_param('base_cache')) ? $base_cache : $TMPL->fetch_param('base_cache');
		$base_cache = $FNS->remove_double_slashes($base_cache);
		
		$img['cache_path'] = $FNS->remove_double_slashes($base_cache.$img['base_rel_path']);
			...
			if(!is_dir($img['cache_path']))
			{
				...
				{
				$TMPL->log_item("imgsizer.Error: could not create cache directory! Please manually create the cache directory ".$img['cache_path']." with 777 permissions");
				return $TMPL->no_results();
				}
			}
			...
			{
				$TMPL->log_item("imgsizer.Error: ".$img['cache_path']." is not writable please chmod 777");
				return $TMPL->no_results();
			}
		...
		foreach($img as $key=>$value){
			$TMPL->log_item("imgsizer.img[".$key."]: ".$value);
		}
...
function do_output($img){
		...
		$justurl = ( ! $TMPL->fetch_param('justurl')) ? '' : $TMPL->fetch_param('justurl');
		$server_domain = ( ! $TMPL->fetch_param('server_domain')) ? '' : $TMPL->fetch_param('server_domain');
		
		$browser_out_path = str_replace(SLASH, "/", $server_domain).$img['browser_out_path'];
		$img['browser_out_path'] = $FNS->remove_double_slashes($browser_out_path);
				...
				foreach ($TMPL->var_single as $key => $val)
				 {
					if ($val == "url")
					{
						$tagdata = $TMPL->swap_var_single($val, $img['browser_out_path'], $tagdata);
					}
					if ($val == "sized")
					{
						$tagdata = $TMPL->swap_var_single($val, $img['browser_out_path'], $tagdata);
					...
					if ($val == "width")
					{
						$tagdata = $TMPL->swap_var_single($val, $img['out_width'], $tagdata);
					}
					if ($val == "img_width")
					{
						$tagdata = $TMPL->swap_var_single($val, $img['out_width'], $tagdata);
					...
					if ($val == "height")
					{
						$tagdata = $TMPL->swap_var_single($val, $img['out_height'], $tagdata);
					}
					if ($val == "img_height")
					{
						$tagdata = $TMPL->swap_var_single($val, $img['out_height'], $tagdata);
...
function get_some_sizes($img){
	global $TMPL,$FNS;
		...
		$max_width = ( ! $TMPL->fetch_param('width')) ? '9000' : $TMPL->fetch_param('width'); 
		$max_height = ( ! $TMPL->fetch_param('height')) ? '9000' : $TMPL->fetch_param('height');
		$greyscale = ( ! $TMPL->fetch_param('greyscale')) ? '' : $TMPL->fetch_param('greyscale');
...
function do_some_image($img) {
	...
	$output = $img['cache_path'].$img['out_name'];
		...
		$quality = ( ! $TMPL->fetch_param('quality')) ? '100' : $TMPL->fetch_param('quality');
		$greyscale = ( ! $TMPL->fetch_param('greyscale')) ? '' : $TMPL->fetch_param('greyscale');
...
function do_remote($img){
		...
		$remote_user = ( ! $TMPL->fetch_param('remote_user')) ? '' : $TMPL->fetch_param('remote_user');
		$remote_pass = ( ! $TMPL->fetch_param('remote_pass')) ? '' : $TMPL->fetch_param('remote_pass');
		
		$url_filename = parse_url($img['src']);
		$url_filename = pathinfo($url_filename['path']);
		
		$base_cache = $FNS->remove_double_slashes($img['base_path']."/images/sized/");
		$base_cache = ( ! $TMPL->fetch_param('base_cache')) ? $base_cache : $TMPL->fetch_param('base_cache');
		$base_cache = $FNS->remove_double_slashes($base_cache);
		...
		$save_rel_path = $FNS->remove_double_slashes($save_rel_path);	
		
		$save_dir = $FNS->remove_double_slashes($base_cache."/remote/");
			...
			if(!is_dir($save_dir))
			{
				...
				{
				$TMPL->log_item("Error: could not create cache directory! Please manually create the cache directory ".$save_dir."/remote/ with 777 permissions");
				return $TMPL->no_results();
				}
			}
		...
		if($remote_diff > $refresh){
				...
				try {
						...
						if (curl_errno($ch)) {
							...
							/*debug*/
							$TMPL->log_item("imgsizer.remote.did_fetch: yes");
							foreach($curl_info as $key=>$value){
								$TMPL->log_item("imgsizer.remote.curl.".$key.": ".$value);
							}
		...
		*/
			...
			/*debug*/
			$TMPL->log_item("imgsizer.remote.save_src_path: ".$save_rel_path);
...
ob_start(); 
?>
Requires GD Be installed on your server

This plugin will resize any JPG, GIF or PNG image to the  size specified
in your EE tag and cache the resized image to the cache folder. If you update
the original image a new resized version will be created. 
...
{exp:imgsizer:size src="/images/news/moped.jpg" width="200" alt="My Title" id="img_5334" class="entry_img"}
     will output
     <img src="/images/sized/images/news/moped-200x134.jpg" width="200" height="134" alt="My Title" class="entry_img" id="img_5334"  />
...
Parameters:
-------------------------------------
src= (required) the relitive path to the image or the URL to the image.
...
quality= (optional for JPG images)  ranges from 0 (worst quality, smaller file) to 100 (best quality, biggest file). The default is the default value is (100).
...
base_cache= (optional) the base cache folder is where all your cache images are stored within sub directories of your base cache folder by default it is "/web/htdocs/lumis.com/images/sized/" you can change this to anything you wish as long as it points to a folder structure in your sites document root 

----experimental----
remote=  set this to "on" if you pass a remote URL through the src= 

refresh= (optional)(in minutes)(only used with remote) this sets how often to update the cache of the remote image default is 1 day
...
to view errors turn on template debugging 
