...
class Freeform_cp_base extends Module_builder_bridge {
	...
	function manage_entries($message='')
    {
		...
		// build crumb
		$this->_build_right_link('export_entries', $this->base . 
	...
	function attachments($message='')
    {
		...
		foreach($query->result_array() as $row)
		{
			...
			$item['filesize']	= ceil($row['filesize'] / 1024)."KB";
			$item['date']		= ee()->localize->set_human_time( $row['entry_date'] );
    ...
    function export_entries($message = '')
    {
		...
		echo ee()->lang->line('count')."\t";

		echo ee()->lang->line('status')."\t";

		echo ee()->lang->line('date')."\t";

		echo ee()->lang->line('form_name')."\t";
		...
		{			
			...
			echo ee()->localize->set_human_time( $row['entry_date'] )."\t";
